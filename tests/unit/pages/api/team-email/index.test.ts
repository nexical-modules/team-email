...
import { GET } from '../../../../../src/pages/api/team-email/index';
import { TeamEmailService } from '../../../../../src/services/team-email-service';
vi.mock('../../../../../src/services/team-email-service');
vi.mock('@/lib/api/api-guard');

describe('TeamEmail API - GET ../../../../../src/pages/api/team-email/index', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call TeamEmailService and return success', async () => {
    ...
    const serviceMethod =
      'GET'.toLowerCase() === 'post'
        ? 'create'
        : 'GET'.toLowerCase() === 'get'
          ? '../../../../../src/pages/api/team-email/index'.includes('[id]')
            ? 'get'
            : 'list'
          : 'GET'.toLowerCase() === 'put'
            ? 'update'
    ...
    vi.mocked((TeamEmailService as Record<string, unknown>)[serviceMethod]).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
