...
describe('TeamEmail API - List', () => {
  ...
  // GET /api/team-email
  describe('GET /api/team-email', () => {
    const baseData = {};

    it('should allow member to list teamEmails', async () => {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const actor = await client.as('user', {});

      // Cleanup first to ensure clean state
      await Factory.prisma.teamEmail.deleteMany();

      // Seed data
      const _listSuffix = Date.now();
      await Factory.create('teamEmail', { ...baseData });
      await Factory.create('teamEmail', { ...baseData });

      const res = await client.get('/api/team-email');
    ...
    it('should verify pagination metadata', async () => {
      ...
      for (let i = 0; i < toCreate; i++) {
        const rec = await Factory.create('teamEmail', { ...baseData });
        createdIds.push(rec.id);
      }

      // Page 1
      const res1 = await client.get('/api/team-email?take=5&skip=0');
      expect(res1.status).toBe(200);
      expect(res1.body.data.length).toBe(5);
      expect(res1.body.meta.total).toBe(15);

      // Page 2
      const res2 = await client.get('/api/team-email?take=5&skip=5');
    ...
    it('should filter by name', async () => {
      ...
      await Factory.create('teamEmail', { ...data1 });
      await Factory.create('teamEmail', { ...data2 });

      const res = await client.get('/api/team-email?name=' + val1);
      expect(res.status).toBe(200);
